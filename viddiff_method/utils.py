...
def create_image_grid_with_labels(
    ...
    if title:
        ...
        draw.text(
            ((grid_width - title_width) / 2, pad_size / 2),
            title,
            fill=(255, 255, 255),
            font=font,
        )
...
def print_strings_on_image(
    ...
    def calculate_image_height(
        string_list, font_size=font_size, line_spacing=line_spacing, max_width=max_width
    ):
        # Use default font
        ...
        for index, text in enumerate(string_list, start=numbering_start_idx):
            ...
            total_lines = (
                wrapped_text.count("\n") + 1
            )  # Counting how many lines will be used
    ...
    image_height = calculate_image_height(
        string_list, font_size, line_spacing, max_width
    )
...
def stack_images(
    ...
    if resize_width:
        ...
        img0 = img0.resize((min_width, int(min_width * img0.height / img0.width)))
        img1 = img1.resize((min_width, int(min_width * img1.height / img1.width)))

    # If resizing by height, adjust both images to the height of the smaller image
    if resize_height:
        min_height = min(img0.height, img1.height)
        img0 = img0.resize((int(min_height * img0.width / img0.height), min_height))
        img1 = img1.resize((int(min_height * img1.width / img1.height), min_height))
...
def add_border_to_img(img, border_position="top", border_size=10):
    ...
    if border_position in ["top", "bottom"]:
        border_img = Image.new("RGB", (img.width, border_size), color=(0, 0, 0))
    else:  # 'left' or 'right'
        border_img = Image.new("RGB", (border_size, img.height), color=(0, 0, 0))
